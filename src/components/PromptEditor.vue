...
import type { PromptVersion as ModelPromptVersion } from './models';
...
// Define the prompt model interface for proper typing
interface PromptModel {
  content: string;
  html: string;
  versions?: ModelPromptVersion[];
...
const props = defineProps({
  modelValue: {
    type: Object as () => PromptModel | undefined,
    default: () => ({
      content: '',
      html: '',
      versions: [],
...
// Markdown output
const markdownOutput = ref(props.modelValue?.content || '');

// Watch for changes to markdownOutput and emit update events
...
// Watch for changes to the editor HTML content
// This is called after the HTML content is updated in the editor
...
// Watch for changes to the modelValue prop from parent
watch(() => props.modelValue, (newValue) => {
  if (newValue && newValue.content !== markdownOutput.value) {
    markdownOutput.value = newValue.content || '';
    
    // Update editor HTML if available and editor is mounted
    if (newValue.html && editorEl.value) {
      editorEl.value.innerHTML = newValue.html;
    }
  }
}, { deep: true });
...
type PromptVersion = ModelPromptVersion;
...
const updateMarkdown = () => {
  ...
  if (editorEl.value) {
    try {
      ...
      // Also update the HTML in the parent component
...
const saveVersion = () => {
  ...
  if (currentVersionIndex.value !== null) {
    ...
    // Update the versions in the parent component
    emit('update:modelValue', {
      ...props.modelValue,
...
const loadVersion = (index: number) => {
  ...
  // Emit update to parent after content is loaded
  emit('update:modelValue', {
    ...props.modelValue,
    content: markdownOutput.value,
    html: editorEl.value.innerHTML,
...
onMounted(() => {
  if (editorEl.value) {
    // Check if we have a pre-existing content from modelValue
    if (props.modelValue?.html) {
      // Use the provided HTML content
      editorEl.value.innerHTML = props.modelValue.html;
    } else {
      // Initialize with a sample prompt
      editorEl.value.innerHTML = '<p>Comece a escrever seu prompt aqui...</p>';
    }
    
    // Initialize versions if available from the parent
    if (props.modelValue?.versions && Array.isArray(props.modelValue.versions) && props.modelValue.versions.length > 0) {
      promptVersions.value = [...props.modelValue.versions];
      if (props.modelValue.currentVersionIndex !== undefined && props.modelValue.currentVersionIndex !== null) {
        currentVersionIndex.value = props.modelValue.currentVersionIndex;
      }
    }
    ...
    setTimeout(() => {
      ...
      // Set current version to null (new unsaved document) if not provided from parent
      if (currentVersionIndex.value === null && !props.modelValue?.currentVersionIndex) {
        currentVersionIndex.value = null;
      }
...
/* Commenting out as this is currently unused
